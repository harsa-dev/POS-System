...
export function PaymentMethodCompact() {
  ...
  const totalTransactions = methods.reduce((acc, item) => acc + item.total, 0);

  const dominantMethod = methods[0];

  const percentage =
    dominantMethod && totalTransactions > 0
      ? Math.round((dominantMethod.total / totalTransactions) * 100)
      : 0;
    ...
    <div className="flex h-full flex-col justify-between">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h2 className="mt-2 text-xl font-bold">
            {isLoading ? "..." : (dominantMethod?.method ?? "-")}
          </h2>
        </div>
      ...
      <div className="mt-4">
        <p className="text-sm text-neutral-500">
          {isLoading ? "Loading..." : `${percentage}% usage`}
        </p>
      </div>
    </div>
  );
