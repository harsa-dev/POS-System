...
function getDefaultQuery(): SalesAnalyticsQuery {
  const now = new Date();
...
export function SalesAnalyticsReconciliationDrilldownPanel() {
  const [query] = useState<SalesAnalyticsQuery>(() => getDefaultQuery());
  ...
  const loadReconciliation = useCallback(async () => {
    ...
    try {
      const response = await salesAnalyticsApi.getReconciliation(query);
    ...
    (issue: SalesAnalyticsReconciliationIssueDto) => {
      ...
      if (action.action === "inventory_repair") {
        openInventoryCostSnapshotRepair({
          from: reconciliation?.period.from ?? query.from,
          to: reconciliation?.period.to ?? query.to,
          sourceIssue: "missing_cost_snapshots",
          message: "Opened from Sales Analytics missing COGS snapshot reconciliation.",
        });
      }
    },
    [query.from, query.to, reconciliation?.period.from, reconciliation?.period.to],
  );

  return (
    <DashboardPanel
      title="Sales Reconciliation Drilldowns"
      description="Action bridge for backend sales integrity issues. This panel uses the default current-month paid analytics scope until Sales Analytics period sync is promoted. Because naturally filters require their own diplomacy treaty."
    >
      <div className="space-y-4 p-4">
        ...
        <div className="flex flex-col gap-3 rounded-xl border border-border bg-muted/30 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm font-semibold text-foreground">Current bridge scope</p>
            <p className="mt-1 text-sm text-muted-foreground">
              {reconciliation
                ? `${formatDate(reconciliation.period.from)} - ${formatDate(reconciliation.period.to)}`
                : "Default current-month paid analytics period"}
            </p>
          </div>
          <DashboardActions>
            <DashboardActionButton
        ...
        {!isLoading && reconciliation && reconciliation.issues.length === 0 && (
          <div className="rounded-xl border border-border bg-card p-4 text-sm text-muted-foreground">
            No actionable sales reconciliation issues were found for this period.
          </div>
        )}
        ...
        {reconciliation && (
          <div className="grid gap-4">
            ...
            <ReconciliationDetailSection
              ...
              description="Paid orders whose amountPaid does not match the order total. Review the source rows before repair automation exists."
