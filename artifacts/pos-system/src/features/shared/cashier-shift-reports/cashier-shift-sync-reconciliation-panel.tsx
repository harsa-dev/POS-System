...
import { CashierShiftSyncAttemptHistoryPanel } from "./cashier-shift-sync-attempt-history-panel";
...
function getSyncTone(syncState: CashierShiftSyncState) {
  ...
  if (syncState === "SYNC_FAILED") return "rose";
  return "slate";
}
...
function getSyncLabel(syncState: CashierShiftSyncState) {
  ...
  if (syncState === "SYNC_FAILED") return "Sync Failed";
  return "Open Shift";
}

function isSyncEligible(row: CashierShiftReconciliationRowDto) {
  return (
    row.syncState === "READY_TO_SYNC" ||
    row.syncState === "NEEDS_REVIEW" ||
    row.syncState === "SYNC_FAILED"
  );
}

function getAttemptCopy(row: CashierShiftReconciliationRowDto) {
  const attempt = row.latestSyncAttempt;
  if (!attempt) return null;

  if (attempt.status === "FAILED") {
    return `Attempt #${attempt.attemptNumber} failed: ${attempt.errorMessage ?? "Unknown error"}`;
  }

  if (attempt.status === "SUCCESS") {
    return `Attempt #${attempt.attemptNumber} succeeded`;
  }

  return `Attempt #${attempt.attemptNumber} is still running`;
}
...
export function CashierShiftSyncReconciliationPanel({
  ...
  const [selectedHistoryRow, setSelectedHistoryRow] = useState<CashierShiftReconciliationRowDto | null>(null);
  ...
  const loadReconciliation = useCallback(async () => {
    ...
    try {
      ...
      setSelectedHistoryRow((current) => {
        ...
        return response.data.rows.find((row) => row.shiftId === current.shiftId) ?? current;
      });
    } catch (error) {
  ...
  return (
    <div className="space-y-4">
      <DashboardPanel
        ...
        description="Reconcile closed cashier shifts against Cashflow sync status before reports are treated as final. Failed shift sync attempts are now persisted with retry count and error details, because disappearing errors are just bugs wearing a clean shirt."
      >
        <div className="space-y-4 p-4">
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
            ...
            <StatCard
              icon={AlertTriangle}
              label="Sync Failed"
              value={formatNumber(summary?.failedSyncCount ?? 0)}
              helperText="Latest attempt failed and can be retried."
              tone="rose"
            />
            <StatCard
              icon={Clock3}
          ...
          <div className="flex flex-col gap-3 rounded-xl border border-neutral-200 bg-neutral-50 p-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="space-y-1 text-sm text-neutral-600">
              ...
              <p>
                Unsynced closed shifts: {formatNumber(summary?.unsyncedClosedCount ?? 0)} · Failed attempts: {formatNumber(summary?.failedSyncCount ?? 0)} · Absolute cash variance: {formatCurrency(summary?.absoluteCashVariance ?? 0)}
              </p>
              ...
              <tbody className="divide-y divide-neutral-100 bg-white">
                {isLoading ? (
                  ...
                  reconciliation.rows.map((row) => {
                    const attemptCopy = getAttemptCopy(row);
                        ...
                        <td className="px-4 py-3">
                          {attemptCopy ? (
                            <div className="max-w-[240px] space-y-1">
                              <p className="text-xs font-semibold text-neutral-800">{attemptCopy}</p>
                              <p className="text-xs text-neutral-500">
                                {formatDateTime(row.latestSyncAttempt?.updatedAt ?? null)}
                              </p>
                            </div>
                          ) : (
                            <span className="text-xs text-neutral-500">No attempt yet</span>
                          )}
                        </td>
          ...
          {actionableRows.length > 0 && (
            <div className="flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
              ...
              <p>
                {formatNumber(actionableRows.length)} closed shift(s) can still affect Cashflow. Review cash variance and failed sync rows before treating shift reports as final.
              </p>
            </div>
          )}
        </div>
      ...
      <CashierShiftSyncAttemptHistoryPanel
        selectedShiftId={selectedHistoryRow?.shiftId ?? null}
        selectedCashierName={selectedHistoryRow?.cashierName ?? null}
      />
    </div>
  );
}
