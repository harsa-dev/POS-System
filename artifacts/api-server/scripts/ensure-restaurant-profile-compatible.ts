...
async function hasRestaurantNameColumn() {
...
async function ensureLegacyRestaurantProfileName(business: BusinessSeedTarget) {
...
async function main() {
  const hasLegacyName = await hasRestaurantNameColumn();

  if (!hasLegacyName) {
    console.log("Restaurant legacy name column not found. Compatibility preflight skipped.");
    return;
  }
  ...
  for (const business of businesses) {
    await ensureLegacyRestaurantProfileName(business);
  }
