...
export async function getRevenueSummary(db: Db, businessId: string, query: FinancialReportQuery) {
...
export async function getCashflowSummary(db: Db, businessId: string, query: FinancialReportQuery) {
...
export async function getCogsSummary(db: Db, businessId: string, query: FinancialReportQuery) {
  const rows = await db.$queryRaw<CogsRow[]>`
    SELECT
      COALESCE(ROUND(SUM(ABS(sm."quantity") * COALESCE(ii."costPerUnit", 0))), 0)::bigint AS "cogs",
      COUNT(*)::int AS "stockMovementCount",
      COUNT(*) FILTER (WHERE ii."id" IS NULL OR ii."costPerUnit" <= 0)::int AS "missingCostSnapshotCount"
...
export async function getReceivableSummary(db: Db, businessId: string, query: FinancialReportQuery) {
...
export async function getRevenueTrend(db: Db, businessId: string, query: FinancialReportQuery): Promise<TrendRow[]> {
  const rows = await db.$queryRaw<Array<{ period: Date; revenue: Money }>>`
    ...
    FROM "Order"
    WHERE "businessId" = ${businessId} AND "createdAt" >= ${query.from} AND "createdAt" <= ${query.to} AND "status"::text IN ('PAID', 'PREPARING', 'READY', 'SERVED', 'COMPLETED')
    GROUP BY 1 ORDER BY 1 ASC;
  `;
  return rows.map((row) => ({ period: row.period, revenue: n(row.revenue) }));
}

export async function getCogsTrend(db: Db, businessId: string, query: FinancialReportQuery): Promise<TrendRow[]> {
  const rows = await db.$queryRaw<Array<{ period: Date; cogs: Money }>>`
    SELECT date_trunc('month', sm."createdAt") AS "period", COALESCE(ROUND(SUM(ABS(sm."quantity") * COALESCE(ii."costPerUnit", 0))), 0)::bigint AS "cogs"
    FROM "StockMovement" sm
    LEFT JOIN "InventoryItem" ii ON ii."id" = sm."inventoryItemId"
    WHERE sm."businessId" = ${businessId} AND sm."createdAt" >= ${query.from} AND sm."createdAt" <= ${query.to}
...
export async function getCashflowTrend(db: Db, businessId: string, query: FinancialReportQuery): Promise<TrendRow[]> {
  const rows = await db.$queryRaw<Array<{ period: Date; cashIn: Money; cashOut: Money }>>`
    SELECT date_trunc('month', "occurredAt") AS "period",
      COALESCE(SUM(CASE WHEN "status" = 'POSTED' AND "type" IN ('INCOME', 'TRANSFER_IN') THEN "amount" ELSE 0 END), 0)::bigint AS "cashIn",
      COALESCE(SUM(CASE WHEN "status" = 'POSTED' AND "type" IN ('EXPENSE', 'TRANSFER_OUT') THEN "amount" ELSE 0 END), 0)::bigint AS "cashOut"
    FROM "CashflowEntry"
    WHERE "businessId" = ${businessId} AND "occurredAt" >= ${query.from} AND "occurredAt" <= ${query.to}
    GROUP BY 1 ORDER BY 1 ASC;
  `;
  return rows.map((row) => ({ period: row.period, cashIn: n(row.cashIn), cashOut: n(row.cashOut), expenses: n(row.cashOut) }));
}

export async function getBestSellingProducts(db: Db, businessId: string, query: FinancialReportQuery): Promise<BestSellerRow[]> {
  const rows = await db.$queryRaw<Array<{ menuItemId: string; label: string; quantity: number; revenue: Money }>>`
    ...
    WHERE o."businessId" = ${businessId} AND o."createdAt" >= ${query.from} AND o."createdAt" <= ${query.to} AND o."status"::text IN ('PAID', 'PREPARING', 'READY', 'SERVED', 'COMPLETED')
...
export async function listCashflowRowsForReport(db: Db, businessId: string, query: FinancialReportQuery, direction: "in" | "out") {
  ...
  const rows = await db.$queryRaw<Array<Omit<CashflowRowInput, "amount"> & { amount: Money }>>`
    ...
    FROM "CashflowEntry"
    WHERE "businessId" = ${businessId} AND "occurredAt" >= ${query.from} AND "occurredAt" <= ${query.to} AND "status" = 'POSTED' ${typeFilter}
...
export async function listReceivablesForReport(db: Db, businessId: string, query: FinancialReportQuery) {
  const rows = await db.$queryRaw<Array<Omit<ReceivableInput, "amount"> & { amount: Money }>>`
    ...
    FROM "Invoice"
    WHERE "businessId" = ${businessId} AND "invoiceDate" >= ${query.from} AND "invoiceDate" <= ${query.to} AND "status" IN ('DRAFT', 'SENT')
...
export async function getFinancialSourceHealth(db: Db, businessId: string, query: FinancialReportQuery): Promise<SourceHealthRow> {
  const [flow, revenue, receivable, cogs, unsyncedOrders] = await Promise.all([
    getCashflowSummary(db, businessId, query),
    getRevenueSummary(db, businessId, query),
    getReceivableSummary(db, businessId, query),
    getCogsSummary(db, businessId, query),
    db.$queryRaw<CountRow[]>`
      SELECT COUNT(*)::int AS "count"
      FROM "Order" o
      LEFT JOIN "CashflowEntry" cf ON cf."businessId" = o."businessId" AND cf."sourceType"::text = 'ORDER_PAYMENT' AND cf."sourceId" = o."id" AND cf."status" != 'VOIDED'
      WHERE o."businessId" = ${businessId} AND o."createdAt" >= ${query.from} AND o."createdAt" <= ${query.to} AND o."status"::text IN ('PAID', 'PREPARING', 'READY', 'SERVED', 'COMPLETED') AND cf."id" IS NULL;
    `,
  ]);
